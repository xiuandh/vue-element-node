...
app.post('/login',urlencode,(request,response)=>{
    ...
    var sql= 'select * from login where admin="'+name+'" and pass="'+password+'"';
     conn.query(sql,(err,result)=>{
        //   console.log(err)
         result.length!=0?response.end('success'): response.end('error');
        // console.log(result);
        // response.end(result)
     })
...
app.get('/removeClasslist',(request,response)=>{
    response.writeHead(200,{
        'content-type':'text/html;charset=utf-8',
        'Access-Control-Allow-Origin':'*',
    });
    var classId=  request.query.id;
    var sql='delete from goodslist where id=" '+classId+' "';
    conn.query(sql,(err,result)=>{
        response.end(JSON.stringify(result));
    })
});
...
app.get('/goods',(request,response)=>{
    ...
    conn.query(sql,(err,result)=>{
        ...
        //查询商拼
        // var sql='select * from goods order by id limit '+start+','+count+' ';
        // var sql='select * from goods ';
        // conn.query(sql,(err,result)=>{
        //       response.end(JSON.stringify(result));
        // })
